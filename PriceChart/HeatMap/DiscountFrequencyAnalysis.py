...
class DiscountFrequencyHeatmap:
        ...
        for i, shop in enumerate(shop_titles):
            ...
            if len(shop_data) > 1:
                ...
                try:
                    ...
                    model = ARIMA(
                        shop_data, order=(1, 1, 1)
                    )  # Tùy chỉnh order (p, d, q) nếu cần
                    model_fit = model.fit()
    ...
    def generate_js(self, output_file="price_chart_heatmap.js"):
        """
        Tạo file js cho biểu đồ heatmap cho Tần Suất Giảm Giá hiện tại và dự đoán Tần Suất Giảm Giá trong 4 tháng tiếp theo.
        ...
        """.format(
                original_chart_json, prediction_chart_json
            )
        )
        ...
        print(f"Scripts đã được lưu vào {output_file}")
...
def main():
    ...
    heatmap_chart.generate_js(
        "PriceChart/HeatMap/price_chart_heatmap.js"
    )  # tạo file js chứa script để vẽ biểu đồ
