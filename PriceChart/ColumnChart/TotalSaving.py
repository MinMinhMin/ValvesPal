...
class TotalSavingsColumnChart:
            ...
            if len(savings) >= 2:
                try:
                    ...
                    predicted_savings[shop_title] = round(
                        total_savings[shop_title] + forecast_total, 2
                    )
                except Exception as e:
    ...
    def generate_js(self, output_file="price_chart_column.js"):
        """
        ...
        """.format(
                historical_chart_json, predicted_chart_json
            )
        )
...
def main():
    ...
    column_chart.generate_js(
        "PriceChart/ColumnChart/price_chart_column.js"
    )  # Tạo file js cho biểu đồ Column Chart
