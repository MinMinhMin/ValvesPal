...
class PriceComparisonGroupedBarChart:
            ...
            for shop in self.shops_data:
                ...
                if relevant_deals:
                    ...
                    if shop_title in latest_prices:
                        time_point_data[date][f"{shop_title}_max"] = latest_prices[
                            shop_title
                        ]["max"]
                        time_point_data[date][f"{shop_title}_min"] = latest_prices[
                            shop_title
                        ]["min"]
                        time_point_data[date][f"{shop_title}_avg"] = latest_prices[
                            shop_title
                        ]["avg"]
                    else:
    ...
    def predict_next_twelve_months(self, processed_data: List[Dict]) -> List[Dict]:
        ...
        for shop_name in shop_names:
            ...
            shop_prices_max = [
                entry["prices"].get(f"{shop_name}_max", None)
                for entry in processed_data
            ]
            shop_prices_min = [
                entry["prices"].get(f"{shop_name}_min", None)
                for entry in processed_data
            ]
            shop_prices_avg = [
                entry["prices"].get(f"{shop_name}_avg", None)
                for entry in processed_data
            ]
            ...
            if len(shop_prices_max) > 1:
                ...
                try:
                    model_max = ARIMA(
                        max_values, order=(1, 1, 1)
                    )  # ARIMA(p,d,q), p, d, q có thể thử các giá trị khác
                        ...
                        predictions.append(
                            {
                                "time_point": date,
                                "shop_name": shop_name,
                                "predicted_max_price": round(
                                    max(0, float(forecast_max[i])), 2
                                ),
                                "predicted_min_price": round(
                                    max(0, float(forecast_min[i])), 2
                                ),
                                "predicted_avg_price": round(
                                    max(0, float(forecast_avg[i])), 2
                                ),
                            }
                        )
                except Exception as e:
        ...
        for prediction in predictions:
            formatted_predictions[prediction["time_point"]][
                f"{prediction['shop_name']}_max"
            ] = prediction["predicted_max_price"]
            formatted_predictions[prediction["time_point"]][
                f"{prediction['shop_name']}_min"
            ] = prediction["predicted_min_price"]
            formatted_predictions[prediction["time_point"]][
                f"{prediction['shop_name']}_avg"
            ] = prediction["predicted_avg_price"]
    ...
    def generate_chart_config(self):
        ...
        for shop_name in shop_names:
            ...
            predicted_max_series.append(
                {"name": f"{shop_name}_max", "data": predicted_max_prices}
            )
            predicted_min_series.append(
                {"name": f"{shop_name}_min", "data": predicted_min_prices}
            )
            predicted_avg_series.append(
                {"name": f"{shop_name}_avg", "data": predicted_avg_prices}
            )
    ...
    def generate_js(self, output_file="price_chart_grouped_bar.js"):
        """
        ...
        """.format(
                original_max_chart_json,
                original_min_chart_json,
                original_avg_chart_json,
                prediction_max_chart_json,
                prediction_min_chart_json,
                prediction_avg_chart_json,
            )
        )
        ...
        print(f"Script đã được lưu vào {output_file}")
...
def main():
    ...
    )  # Tạo file js chứa cấu hình biểu đồ
