...
class ChartStage(QMainWindow):
    ...
    def display(self):
        ...
        if self.type == "steamUsers":
            ...
            self.web_view.load(QUrl("http://127.0.0.1:5500/barRaceCharts/BarRace_all.html"))
            chartStr = self.type + " games"
            self.chartName.setText("Bar chart showing the monthly ranking of "+ chartStr +" from November 2021 to November 2024")
...
class gameStage(QMainWindow):
    def __init__(self, item,pixmap :QPixmap, parent):
        ...
        fixed_pixelfont([
            self.steamScore,self.steamCount,self.metaScore,self.metaCount,self.metaUserScore,self.metaUserCount,
            self.openCriticScore,self.openCriticScore,self.openCriticCount,self.recentPlayerCount,self.dayPlayerCount,self.weekPlayerCount,
            self.peak,self.label_11
        ],20)
    ...
    def display(self,check:dict):
        ...
        self.releaseDate.setText("Ngày phát hành: "+ str(item.details.info.releaseDate))
        tags_str = "Tất cả các tags: "
        tags = item.details.info.tags
        for tag in tags:
            if tag != tags[-1]: tags_str += tag +", "
            else: tags_str += tag
        ...
        self.recentPlayerCount.setText("Lượng người chơi hiện tại: " + str(playerInfo.recent))
        self.dayPlayerCount.setText("Lượng người chơi trong ngày: " + str(playerInfo.day))
        self.weekPlayerCount.setText("Lượng người chơi trong tuần: " + str(playerInfo.week))
        ...
        if item.deal.price != "Free":
            ...
            saleStr = str(int(100-(item.deal.price.amount/item.deal.regular.amount)*100)) +"%"
        ...
        self.closeButton.setStyleSheet("""
            ...
            """)
        fullScreenButtons = [self.playCountFull,
       ...
       #chart playerCount 
        ...
        if check["DF"] == True: 
            price_chart_step_line_Url = "http://127.0.0.1:5500/DiscountFrequencyAnalysis/price_chart_step_line.html"
        ...
        self.price_chart_step_lineContainer = self.findChild(QWidget, "price_chart_step_line")
        ...
        if check["DF"] == True: 
            price_chart_pie_chart_Url = "http://127.0.0.1:5500/DiscountFrequencyAnalysis/price_chart_pie_chart.html"
        ...
        self.price_chart_pie_chartContainer = self.findChild(QWidget, "price_chart_pie_chart")
        ...
        if check["DF"] == True: 
            price_chart_heatmap_Url = "http://127.0.0.1:5500/DiscountFrequencyAnalysis/price_chart_heatmap.html"
        self.price_chart_heatmap_web_view = QWebEngineView(self)
        self.price_chart_heatmap_web_view.load(QUrl(price_chart_heatmap_Url))
        self.price_chart_heatmapContainer = self.findChild(QWidget, "price_chart_heatmap")
        ...
        if check["DF"] == True: 
            price_chart_grouped_bar_Url = "http://127.0.0.1:5500/DiscountFrequencyAnalysis/price_chart_grouped_bar.html"
        ...
        self.price_chart_grouped_barContainer = self.findChild(QWidget, "price_chart_grouped_bar")
        price_chart_grouped_bar_Layout = QVBoxLayout(self.price_chart_grouped_barContainer)  
        ...
        if check["DF"] == True: 
            price_chart_column_chart_Url = "http://127.0.0.1:5500/DiscountFrequencyAnalysis/price_chart_column_chart.html"
        ...
        self.price_chart_column_chartContainer = self.findChild(QWidget, "price_chart_column_chart")
        price_chart_column_chart_Layout = QVBoxLayout(self.price_chart_column_chartContainer)  
        self.price_chart_column_chartContainer.setLayout(price_chart_column_chart_Layout)
        ...
        """)
        price_chart_column_chart_Layout.addWidget(self.price_chart_column_chart_web_view)

        #chart  price_chart_boxplot
        price_chart_boxplot_Url = "http://127.0.0.1:5500/playersCharts/ERROR.html"
        if check["DF"] == True: 
            price_chart_boxplot_Url = "http://127.0.0.1:5500/DiscountFrequencyAnalysis/price_chart_boxplot.html"
        self.price_chart_boxplot_web_view = QWebEngineView(self)
        self.price_chart_boxplot_web_view.load(QUrl(price_chart_boxplot_Url))
        self.price_chart_boxplotContainer = self.findChild(QWidget, "price_chart_boxplot")
        ...
        self.price_chart_boxplot_FullScreenStage = ChartFullScreen(price_chart_boxplot_Url)
        self.price_chart_column_chart_FullScreenStage = ChartFullScreen(price_chart_column_chart_Url)
        self.price_chart_grouped_bar_FullScreenStage = ChartFullScreen(price_chart_grouped_bar_Url)
        self.price_chart_heatmap_FullScreenStage = ChartFullScreen(price_chart_heatmap_Url)
        self.price_chart_pie_chart_FullScreenStage = ChartFullScreen(price_chart_pie_chart_Url)
        self.price_chart_step_line_FullScreenStage = ChartFullScreen(price_chart_step_line_Url)

        self.playCountFull.clicked.connect(self.playerCountFullScreenStage.showFullScreen)
        self.price_chart_boxplotFull.clicked.connect(self.price_chart_boxplot_FullScreenStage.showFullScreen)
        self.price_chart_column_chartFull.clicked.connect(self.price_chart_column_chart_FullScreenStage.showFullScreen)
        self.price_chart_grouped_barFull.clicked.connect(self.price_chart_grouped_bar_FullScreenStage.showFullScreen)
        self.price_chart_heatmapFull.clicked.connect(self.price_chart_heatmap_FullScreenStage.showFullScreen)
        self.price_chart_pie_chartFull.clicked.connect(self.price_chart_pie_chart_FullScreenStage.showFullScreen)
        self.price_chart_step_lineFull.clicked.connect(self.price_chart_step_line_FullScreenStage.showFullScreen)
    ...
    def run(self):
        try:
            ...
            if gamecount is not None or gamePriceData is not None:
                ...
                if gamecount.check : checked["PC"] = True
                if gamePriceData : checked["DF"] = True
                self.data_loaded.emit(checked)
            else: self.data_unloaded.emit()
        except Exception as e :
            print(e)
...
class MainStage(QMainWindow):
    def __init__(self):
        ...
        chartRace_names = ["chartsAll", "chartsSinglePlayer", "chartsAnime","chartsRacing","chartsSandbox",
                           "chartsVisualNovel","chartsSports","chartsHorror","SteamUserChart"]
    ...
    def display_games(self, daily_game):
        ...
        for item in daily_game.game_list:
            ...
            if pixmap:
                image_game.setPixmap(pixmap.scaled(100, 100, Qt.AspectRatioMode.KeepAspectRatio))
            ...
            game_name.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            ...
            curentPrice.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            ...
            basePrice.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
            basePrice.setMinimumHeight(90)
            basePrice.setEnabled(False)
            basePrice.setStyleSheet("background-color: rgba(0, 0, 0, 0);")

            
            sale = QLabel("Giảm: " + str(int(100-(item.deal.price.amount/item.deal.regular.amount)*100)) + " %" )
            ...
            text_container.mousePressEvent = lambda event, item = item, pixmap=pixmap: self.game_clicked(item, pixmap)
    ...
    def game_clicked(self, item,pixmap: QPixmap):
        ...
        self.new_stage.setWindowModality(
                Qt.WindowModality.ApplicationModal
            )  
        self.new_stage.show()
